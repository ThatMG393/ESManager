...
public class CrashActivity extends AppCompatActivity {
    ...
    private void setupCrashDialog() {
        LayoutInflater li =
                LayoutInflater.from(
                        new ContextThemeWrapper(getApplicationContext(), R.style.App_Main));
        View layoutV = li.inflate(R.layout.activity_crash_dialog, null);
    ...
    private void setupCrashDialogView(View v) {
        ...
        ((TextView) v.findViewById(R.id.crash_comment_hint)).setText("Why did it happen? What did you do to make it happen?");

        ((TextView) v.findViewById(R.id.crash_overview))
                .setText("Caused by: " + getIntent().getStringExtra("exceptionOverview"));
        TextView tv = v.findViewById(R.id.crash_logs);
        tv.setText(getIntent().getStringExtra("exceptionStackTrace"));
        tv.setMovementMethod(new ScrollingMovementMethod());
        ((Button) v.findViewById(R.id.crash_view_logs))
                .setOnClickListener(
    ...
    private void toggleLogsVisibility(View v) {
        if (isUserViewingLogs) {
            ...
            ((LinearLayout) v.findViewById(R.id.crash_comment_container))
    ...
    private void uploadLog(View v) {
        ...
        StorageMetadata logMetadata =
                new StorageMetadata.Builder()
                        .setContentType("text/plain")
                        .setContentEncoding("utf-8")
                        .build();

        logFileSR.putFile(logFileUri, logMetadata)
                .addOnSuccessListener(
                        new OnSuccessListener<UploadTask.TaskSnapshot>() {
                            @Override
                            public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                                ((ImageView) v.findViewById(R.id.crash_upload_icon))
                                        .setVisibility(View.VISIBLE);
                                ((CircularProgressIndicator) v.findViewById(R.id.crash_upload_pb))
                                        .setProgress(0);
                                ((CircularProgressIndicator) v.findViewById(R.id.crash_upload_pb))
                                        .setVisibility(View.GONE);
                                ((TextView) v.findViewById(R.id.crash_upload_text))
                                        .setText("Uploaded log file successfully!\nExiting in 4s");
                ...
                .addOnProgressListener(
                        new OnProgressListener<UploadTask.TaskSnapshot>() {
                            ...
                            public void onProgress(UploadTask.TaskSnapshot taskSnapshot) {
                                ...
                                ((CircularProgressIndicator) v.findViewById(R.id.crash_upload_pb))
                                        .setProgress((int) uploadProgress);
                                ((TextView) v.findViewById(R.id.crash_upload_text))
                                        .setText("Uploading... (" + uploadProgress + "% / 100%)");
                            }
                        });
    }
    ...
    private void createLogOnLogsDir() {
        String logsDir =
                Utils.FileUtils.getExternalFilesDirPaths(
                        getApplicationContext(), Utils.FileUtils.LOGS_FOLDER);
        File logFile = new File(logsDir + "/" + currentDate + ".log");
        try (FileWriter openedLogFile = new FileWriter(logFile); ){
            logFile.createNewFile();
            openedLogFile.write(constructLog());
			
            logFilePath = logFile.getAbsolutePath();
        } catch (IOException ioe) {
            // Utils.LoggerUtils.logErr("ESManager/CrashActivity", ioe.toString());
        }
    }
